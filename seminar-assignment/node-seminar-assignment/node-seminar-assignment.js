// EN: Hub for seminar-assignment. Same `handleIncoming` + `broadcastTo`
//     pattern as the joined-example-* hubs. Browser sends `LED`
//     messages (button clicks) → forwarded to the ESP32 on port 8811.
//     ESP32 streams `fotoupornik` readings and `status` changes →
//     forwarded to all browsers on port 8888. The last `status` is
//     cached so newly-connected browsers see the current state
//     immediately (the chip only emits status on CHANGE).
// SL: Zvezdišče za seminar-assignment. Enak vzorec `handleIncoming` +
//     `broadcastTo` kot v hub-ih joined-example-*. Brskalnik pošlje
//     sporočila `LED` (kliki gumbov) → posredovano ESP32-ju na vrata
//     8811. ESP32 pošilja meritve `fotoupornik` in spremembe `status`
//     → posredovane vsem brskalnikom na vrata 8888. Zadnji `status`
//     se hrani v predpomnilniku, da novo povezan brskalnik takoj
//     vidi trenutno stanje (čip status pošlje le ob SPREMEMBI).
...
app.get("/", (req, res) => {
    res.sendFile(path.join(__dirname, "node-seminar-assignment.html"));
});
