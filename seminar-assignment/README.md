# Seminar assignment — Closed-loop LED control with photoresistor feedback
...
├── node-seminar-assignment/          ← Node side
│   ├── node-seminar-assignment.js      ← Express + two WSS + status cache
│   ├── node-seminar-assignment.html    ← Browser UI (buttons + chart + banner)
│   └── package.json
└── README.md
```
...
**1. Node hub.** From `seminar-assignment/node-seminar-assignment/`:
...
**2. ESP32.** From `seminar-assignment/`:
...
- **`EACCES: permission denied 0.0.0.0:80`** → port 80 needs admin on Windows. Run PowerShell as admin or change `HTTP_PORT` in `node-seminar-assignment.js` to 8080.
...
├── node-seminar-assignment/          ← Node stran
│   ├── node-seminar-assignment.js      ← Express + dva WSS + predpomnilnik stanja
│   ├── node-seminar-assignment.html    ← brskalniški vmesnik (gumbi + graf + pas)
│   └── package.json
└── README.md
```
...
**1. Node zvezdišče.** V mapi `seminar-assignment/node-seminar-assignment/`:
...
**2. ESP32.** Iz mape `seminar-assignment/`:
...
- **`EACCES: permission denied 0.0.0.0:80`** → vrata 80 na Windowsih potrebujejo skrbniške pravice. Zaženi PowerShell kot skrbnik ali spremeni `HTTP_PORT` v `node-seminar-assignment.js` na 8080.
